...
from TipForceReferenceResults import AlphaS4R, UtipS4R, WtipS4R
...
alphaPlot = np.linspace(0.,1.0,101)*4

thetaTip, XDispNorm, ZDispNorm = analyticCantileverDisplacement(alphaPlot)

fig, ax = plt.subplots(figsize=(9,5))

ax.plot(alphaPlot, ZDispNorm, label="Analytic", clip_on=False, c=colours["Axis"], lw=4)
ax.plot(AlphaS4R, WtipS4R/LENGTH,'o', label="Abaqus S4R", clip_on=False, markersize=10)
...
for strainType in ["nonlinear"]: # "linear"
    for rotationType in ["linear", "quadratic"]:
        try:
            ...
            ax.plot(TACSDisps["tipForce"]*4, TACSDisps["zDisp"]/LENGTH,'o', label=f"TACS, {strainType} strain, {rotationType} rotation", clip_on=False, markersize=10)
        except FileNotFoundError:
            pass

ax.set_xlabel(r"$\frac{F_{tip}L^2}{EI}$", fontsize=24)
ax.set_ylabel(r"$\frac{\Delta Z_{tip}}{L}$", rotation="horizontal", ha="right", fontsize=24)
...
niceplots.save_figs(fig, "TipForceValidation", ["png", "pdf", "svg"], bbox_inches="tight")
